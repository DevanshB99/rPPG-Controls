...
OUT_CSV    = Path("/home/macs/Documents/rPPG-Controls/rppg/bpm_bisenet_output.csv")
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if mp_res.face_landmarks:
            ...
            fore_skin = comb_sk & fm   & ~hair_fore  & ~excl
            l_skin    = comb_sk & lcm  & ~hair_cheek & ~excl
            r_skin    = comb_sk & rcm  & ~hair_cheek & ~excl
            ...
            if n_px >= 50:
                ...
                R_t.append(float(fn[:,:,0][final].mean()))
                G_t.append(float(fn[:,:,1][final].mean()))
                B_t.append(float(fn[:,:,2][final].mean()))
    ...
    print(f"\nPipeline quality summary across {T} frames:")
    print(f"  quality_score  mean={q.mean():.3f}  std={q.std():.3f}  "
          f"p10={np.percentile(q,10):.3f}  p90={np.percentile(q,90):.3f}")
    print(f"  bisenet_conf   mean={np.mean(bisenet_conf_t):.3f}")
    print(f"  vote_mean      mean={np.mean(vote_mean_t):.3f}  (1.0 = all 3 colour methods agree)")
    print(f"  clust_agree    mean={np.mean(clust_agree_t):.3f}  (GMM consensus)")
    print(f"  roi_coverage   mean={np.mean(roi_coverage_t):.3f}  (skin fraction of face ROI)")
    print(f"  green_cv       mean={np.mean(green_cv_t):.3f}  (< 0.10 = spatially uniform)")
    print(f"  hair_rej_frac  mean={np.mean(hair_rej_frac_t):.3f}  (fraction removed as hair)")
